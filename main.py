...
class Processor():
    def __init__(self, arg):
        ...
        if getattr(self.arg, 'use_wandb', False):
            import wandb
            # Setup wandb run if the flag is provided in configs
            wandb_kwargs = {
                "project": getattr(self.arg, 'wandb_project', 'SEN_CSLR'),
                "name": getattr(self.arg, 'wandb_name', None),
                "config": vars(self.arg)
            }
            if getattr(self.arg, 'wandb_id', None):
                wandb_kwargs["id"] = self.arg.wandb_id
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                # train end2end model
                train_loss_list = seq_train(self.data_loader['train'], self.model, self.optimizer,
    ...
    def loading(self):
        ...
        if getattr(self.arg, 'freeze_conv2d', False):
            print("Freezing conv2d backbone...")
            for param in model.conv2d.parameters():
                param.requires_grad = False

        #optimizer = utils.Optimizer(adjust_lr_params, self.arg.optimizer_args)
        optimizer = utils.Optimizer(filter(lambda p: p.requires_grad, model.parameters()), self.arg.optimizer_args)
    ...
    def load_model_weights(self, model, weight_path):
        ...
        model.load_state_dict(weights, strict=strict_load)
